...
io.on('connection', (socket) => {
	socket.on('join', (data) => {
		...
		if (!Object.keys(allsockets).includes(room)) {
			...
			socket.emit('message', {
				name: 'server',
				message: `Welcome to the Go-Knights-Productions Public Chat! You are currently in room "${socket.proto.room}". To join a different room, type /join [room]. Type /help to see more commands`
			});
