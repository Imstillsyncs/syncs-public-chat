...
function query(obj, and, db) {
  ...
  for (let i in keys) {
    if (i > 0 && and) {
      Object.keys(main)
        .filter((el) => db[el].proto[keys[i]] === values[i])
        .map((el) => (ret[el] = db[el]));
    } else {
      Object.keys(db)
        .filter((el) => db[el].proto[keys[i]] === values[i])
        .map((el) => (main[el] = db[el]));
    }
  }
  if (and) {
    return ret;
...
io.on('connection', (socket) => {
  socket.on('join', (data) => {
    ...
    if (!Object.keys(allsockets).includes(room)) {
      ...
      socket.emit('message', {
        name: 'server',
        message: `Welcome to the Go Knights Productions Chat Room! You are currently in room "${socket.proto.room}". To join a different room, type /join [room]. Type /help to see more commands`,
      });
        ...
        switch (message.split(' ')[0]) {
          case '/whois':
            ...
            if (selectedSocket) {
              socket.emit('message', {
                name: 'server',
                message: `<br>${selectedSocket.proto.id} is ${
                  selectedSocket.proto.name
                }<br>
								${selectedSocket.proto.name} is in ${rooms.length} room${
                  rooms.length > 1 ? 's' : ''
                }: ${rooms.join(', ')}<br>
              ...
              message.split(' ')[1] &&
              message.split(' ')[1].indexOf('#') === 0
            ) {
              selectedSocket = query({
                room: message.split(' ')[1].substr(1),
              });
              rooms = Object.keys(selectedSocket).map(
                (el) => selectedSocket[el].proto.name
              );
              socket.emit('message', {
                name: 'server',
                message: `${message.split(' ')[1]} has ${rooms.length} user${
                  rooms.length > 1 ? 's' : ''
                }: ${rooms.join(', ')}`,
              });
            } else {
          ...
          case '/deop':
            ...
            if (socket.proto.admin) {
              if (
                selectedSocket &&
                selectedSocket.proto.id !== socket.proto.id
              ) {
          ...
          case '/users':
            socket.emit('message', {
              ...
              message: queryKeys({
                ...
                .map((el) => sockets[el].proto.name)
                .join(', ')
                .replace(socket.proto.name, '<b>$&</b>'),
            });
            break;
          case '/rooms':
            rooms = occurences(
              Object.keys(allsockets).map((el) => allsockets[el].proto.room)
            );
            let users = rooms.b;
            rooms = rooms.a;
            socket.emit('message', {
              ...
              message: rooms
                .map(
                  (el, i) =>
                    `<a href="/${el}" target="_self">${el}</a> (${users[i]})`
                )
                .join(', '),
            });
            break;
